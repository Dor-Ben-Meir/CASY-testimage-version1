...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Emulated_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    Emulated_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(Emulated_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Emulated_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(Emulated_M, 0);
