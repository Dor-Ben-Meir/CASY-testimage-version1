...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(pCKAN_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    pCKAN_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(pCKAN_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(pCKAN_M, "stopping the model");
  runModel = 0;
}
