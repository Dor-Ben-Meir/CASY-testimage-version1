 ...
 * Component: Peservoblock
 * Component Simulink Path: Peservoblock
 * Simulation Mode: PIL
 *
 */

#define S_FUNCTION_NAME                Peservoblock_pbs
    ...
    if ((pIsXILApplicationStarted!=NULL) && (*pIsXILApplicationStarted==1)) {
...

      {
        ...
        if (IOBufferPtr != NULL) {
          void * pXILService = (void *) ssGetPWorkValue(S, 9);
          if (pXILService != NULL) {
            if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 77, 0)!=
                XILHOSTAPPSVC_SUCCESS) {
              return;
            }                          /* if */
              ...
              *mxMemUnitPtr = (uint8_T) XIL_PROCESS_PARAMS_COMMAND;
...

              {
...

                {
                  uint8_T * simDataMemUnitPtr;
                  uint32_T offsetIdx;
                  for (offsetIdx=0; offsetIdx< 9; offsetIdx++) {
...

                    {
                      ...
                      simDataMemUnitPtr = (uint8_T*) (((char *)
                        simDataMemUnitPtr) +
                        dtaGetDataTypeElementOffset(dta, bpath, ssGetDataTypeId
                        (S, "struct_KbuHZdFirwgp0Ajtitq2dC"), offsetIdx));

                      {
                        DTypeId dType = ssGetDataTypeId(S,
                          "struct_KbuHZdFirwgp0Ajtitq2dC");
                        size_t num_elements;
      ...
      {
        {
          ...
          if (commandDispatch(S, IOBufferPtr, 77)!=XILHOSTAPPSVC_SUCCESS) {
            return;
          }                            /* if */
...
{
  {
    ...
    dims[0] = 1;
    dims[1] = 1;
    ssCheckSFcnParamValueAttribs(S, 0, "Flight_Logic", DYNAMICALLY_TYPED, 2,
      dims, 0);
  }
}
...
{
  {
    ...
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 101, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */
...
static void mdlSetWorkWidths(SimStruct *S)
{
  ...
  {
    uint32_T dtChecksum[4] = { 898828658U, -1000196573U, -1164725858U,
      1456328882U };

    DTypeId dataTypeId = INVALID_DTYPE_ID;
    ssGetSFcnParamDataType(S, 0, &dataTypeId);
    if (dataTypeId == INVALID_DTYPE_ID)
      return;
    ssCheckStructParamChecksum(S, 0, dtChecksum);
    ssRegDlgParamAsRunTimeParam(S, 0, 0, "P0_Flight_Logic", dataTypeId);
    if (ssGetErrorStatus(S) != (NULL))
      return;
  }
}
