...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(g232F_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    g232F_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(g232F_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(g232F_M, "stopping the model");
  runModel = 0;
}
