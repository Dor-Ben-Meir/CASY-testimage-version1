...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(PwmBox_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    PwmBox_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(PwmBox_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(PwmBox_M, "stopping the model");
  runModel = 0;
}
