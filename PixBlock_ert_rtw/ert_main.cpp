...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(PixBlock_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    PixBlock_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(PixBlock_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(PixBlock_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(PixBlock_M, 0);
