...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(GPS32_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    GPS32_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(GPS32_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(GPS32_M, "stopping the model");
  runModel = 0;
}
