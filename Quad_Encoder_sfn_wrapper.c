 ...
 *
 */
void Quad_Encoder_sfn_Outputs_wrapper(int16_T *pos,
			const real_T *xD,
 ...
 *
 */
void Quad_Encoder_sfn_Update_wrapper(int16_T *pos,
			real_T *xD,
			const uint8_T *enc, const int_T p_width0,
			const uint8_T *pinA, const int_T p_width1,
			const uint8_T *pinB, const int_T p_width2)
...
if (xD[0]!=1) {
    ...
    # ifndef MATLAB_MEX_FILE
        ...
        switch(enc[0]) {
          case 0:
			 wiringPiISR (Enc[0].pinA, INT_EDGE_BOTH, &irsPinAEn0);
			 wiringPiISR (Enc[0].pinB, INT_EDGE_BOTH, &isrPinBEn0);
            break;  
          case 1:
            wiringPiISR (Enc[1].pinA, INT_EDGE_BOTH, &irsPinAEn1);
            wiringPiISR (Enc[1].pinB, INT_EDGE_BOTH, &isrPinBEn1);
            break;                                   
          case 2:                                    
            wiringPiISR (Enc[2].pinA, INT_EDGE_BOTH, &irsPinAEn2);
            wiringPiISR (Enc[2].pinB, INT_EDGE_BOTH, &isrPinBEn2);
            break;  
          }
