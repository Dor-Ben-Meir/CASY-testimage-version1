...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(P2R5_M) == (NULL));
  while (runModel) {
    sem_wait(&baserateTaskSem);
    P2R5_step();

    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(P2R5_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(P2R5_M, "stopping the model");
  runModel = 0;
}
